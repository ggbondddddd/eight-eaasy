...
// 方法1，就是用flat方法，Infinity就是无限层扁平化
       ...
       console.log(arr5);
        ...
        // 方法2 利用while 配合concat，因为concat可以合并数组和一些单个属性为一个新数组如concat（1，[2,3]） 结果为[1,2,3]   首先当some方法判断出数组某一个属性为数组返回true，while内部上下文触发，扩展运算符展开一层arr
        let arr4 = [1, [2, [3, 4]]];
        function flat(arr) {
...
#### 2.echarts地图自定义地图引入（有一个问题，深山合作区的经纬度其实是不对的）
